...
Replace `index.html` **inside the repo** and push. That is the only supported way
to ship a new version.

Do not drag the single `index.html` onto Netlify's deploy drop zone. A drag and
drop deploy replaces the entire published site with whatever you dropped, so the
functions, `netlify.toml` and `package.json` disappear with it. The page still
loads and still looks right, but `/api/targets` starts answering 404, the header
badge flips from **Shared** to **This browser**, and every upload and target edit
quietly stays in that one browser.

The page now detects this itself: a red banner across the top names the status
code and the missing files. To confirm either way, open
`https://<your-site>/api/targets` in a tab. JSON (or `null`) means the functions
are live. A Netlify 404 page means they are gone and the folder needs redeploying.
If you dropped a single file, the fix is to redeploy the whole folder, or push to
the repo and let Netlify build; no data is lost, since the Blobs store survives a
bad deploy.
